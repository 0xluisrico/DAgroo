...
contract Main is ERC721, Ownable, ReentrancyGuard{
    ...
    struct Uri {
        string uriLandPreparation;
        string uriSeeding;
        string uriGermination;
        string uriGrowth;
        string uriMaduration;
        string uriHarvest;
        string uriProcessing;
        string uriPackaging;
        string uriStorage;
        string uriDistribution;
        string uriDelivery;
    }
    ...
    struct TraceabilityAsset {
        address owner;
        uint256 fightId;
    }

    mapping(uint256 => TraceabilityAsset) traceabilityAssets;
    ...
    event GrowerCreated(address indexed grower, string indexed name, uint256 indexed timestamp);
    ...
    modifier OnlyBuyer() {
        require(isBuyer[msg.sender] == true, "No eres buyer");
        _;
    }
    ...
    constructor()ERC721("Traceability Asset", "DAGRO"){
        platformFee = 40;
    }
    ...
    function createGrower(string calldata _name, string calldata _ubication) public {
        ...
        emit GrowerCreated(msg.sender, _name, block.timestamp);
    }
    ...
    function BuyProduct(uint256 _idProduct) public payable OnlyBuyer nonReentrant{
    ...
    function traceabilityAsset(address _buyer, uint256 _idProduct, productState _state) internal {
        string memory stateUri;

        tokenIds.increment();

        uint256 traceabilityAssetId = tokenIds.current();


        if (_state == productState.LAND_PREPARATION) {
            stateUri = uri.uriLandPreparation;
        } else if (_state == productState.SEEDING) {
            stateUri = uri.uriSeeding;
        } else if (_state == productState.GERMINATION) {
            stateUri = uri.uriGermination;
        } else if (_state == productState.GROWTH) {
            stateUri = uri.uriGrowth;
        } else if (_state == productState.MADURATION) {
            stateUri = uri.uriMaduration;
        } else if (_state == productState.HARVEST) {
            stateUri = uri.uriHarvest;
        } else if (_state == productState.PROCESSING) {
            stateUri = uri.uriProcessing;
        } else if (_state == productState.PACKAGING) {
            stateUri = uri.uriPackaging;
        } else if (_state == productState.STORAGE) {
            stateUri = uri.uriStorage;
        } else if (_state == productState.DISTRIBUTION) {
            stateUri = uri.uriDistribution;
        } else if (_state == productState.DELIVERY) {
            stateUri = uri.uriDelivery;
        }
        
        _mint(_buyer, traceabilityAssetId);
        _setTokenURI(traceabilityAssetId, stateUri);
        
        traceabilityAssets[traceabilityAssetId] = TraceabilityAsset(msg.sender, _idProduct);

        emit TraceabilityAssetEvent(_buyer, _idProduct, traceabilityAssetId, block.timestamp);
    }
    ...
    function changeStateProduct(productState _state, uint256 _id) public OnlyGrower{
        ...
        traceabilityAsset(product.buyer, _id, product.proState);
        emit StateChanged(_id, _state, block.timestamp);
    }

    function setUris(string memory _uriLandPreparation, string memory _uriSeeding, string memory _uriGermination,
    string memory _uriGrowth, string memory _uriMaduration, string memory _uriHarvest, string memory _uriProcessing,
    string memory _uriPackaging, string memory _uriStorage, string memory _uriDistribution, string memory _uriDelivery)
    public onlyOwner{
        uri.uriLandPreparation = _uriLandPreparation;
        uri.uriSeeding = _uriSeeding;
        uri.uriGermination = _uriGermination;
        uri.uriGrowth = _uriGrowth;
        uri.uriMaduration = _uriMaduration;
        uri.uriHarvest = _uriHarvest;
        uri.uriProcessing = _uriProcessing;
        uri.uriPackaging = _uriPackaging;
        uri.uriStorage = _uriStorage;
        uri.uriDistribution = _uriDistribution;
        uri.uriDelivery = _uriDelivery;
    }
